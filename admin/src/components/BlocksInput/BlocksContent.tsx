...
import { useDragAndDrop, DIRECTIONS } from "./utils/useDragAndDrop";
import { ItemTypes } from "./utils/dragAndDrop";
import { composeRefs } from "./utils/refs";
import { getTranslation } from "./utils/translations";

import { type BlocksStore, useBlocksEditorContext } from ".";
...
const DragAndDropElement = ({
  ...
  const { editor, disabled, name, setLiveText } =
  ...
  return (
    ...
    >
      ...
      {isDragging ? (
        <CloneDragItem dragHandleTopMargin={dragHandleTopMargin}>
          {children}
        </CloneDragItem>
      ) : (
        <DragItem
...
const BlocksContent = ({ placeholder, ariaLabelId }: BlocksContentProps) => {
  ...
  const handleMoveBlocks = (
    ...
    if (event.key === "ArrowUp") {
      ...
      newIndexPosition =
        currentIndex[0] < editor.children.length - 1
          ? currentIndex[0] + 1
          : currentIndex[0];
    }
